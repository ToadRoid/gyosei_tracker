...
export async function getSubjectStats(): Promise<SubjectStats[]> {
  ...
  return subjects.map((subject) => {
    const subjectAttrs = allAttrs.filter((a) => a.subjectId === subject.id && a.isExcluded !== true && a.needsSourceCheck !== true && a.aiTriageStatus !== 'discard');
...
export async function getChapterStats(): Promise<ChapterStats[]> {
  ...
  return chapters.map((chapter) => {
    const chapterAttrs = allAttrs.filter((a) => a.chapterId === chapter.id && a.isExcluded !== true && a.needsSourceCheck !== true && a.aiTriageStatus !== 'discard');
...
export async function getOverallStats() {
  ...
  const totalReady = readyProblems.filter((p) => {
    ...
    return !attr || (attr.isExcluded !== true && attr.needsSourceCheck !== true);
  }).length;
