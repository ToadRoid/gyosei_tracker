...
信頼度レベルと運用ルール (2026-04-07 確定):
  HIGH  (conf ≥ 0.90) → --apply で自動修正 : カテゴリC のみ
        「本肢は正しい」「本肢は誤り」等の明示宣言を全文スキャン
  MEDIUM(conf 0.60〜0.89) → レポートのみ、自動適用禁止 : カテゴリA,B,F
        末尾パターン・類似度ベース。誤検出率 78% (2026-04-07 手動照合実測)
        → 全文確認後、db.ts の recalcCorrect に個別追記して適用すること
  LOW   (フラグのみ) → needsSourceCheck 候補 : カテゴリD,E (OCR/品質問題)
"""
...
REPORT_PATH = ROOT / "data" / f"import_optimizer_{_RUN_TS}.json"
CSV_DIFF_PATH = ROOT / "data" / f"import_optimizer_diff_{_RUN_TS}.csv"
...
def run_optimizer(apply: bool = False, verbose: bool = False, _save_report: bool = True) -> dict:
        ...
        for br in page.get('branches', []):
            ...
            if direction != 'ambiguous' and a_bool is not None:
                ...
                if expected != a_bool:
                    ...
                    if confidence >= 0.90:
                        # ──────────────────────────────────────────────
                        # 運用ルール: カテゴリC (conf ≥ 0.90) のみ --apply で自動修正。
                        # カテゴリA/B/F (conf 0.60〜0.89) はレポートのみ。自動適用禁止。
                        # 根拠: 2026-04-07 全178件手動照合で A/B/F の誤検出率 78% を確認。
                        #       「本肢は正しい/誤り」等の明示宣言のみが信頼できる判定根拠。
                        # ──────────────────────────────────────────────
                        if apply:
                        ...
                        # MEDIUM (A/B/F): レポートのみ、自動適用しない
                        # → 人間が全文照合して確認後、db.ts の recalcCorrect で個別適用すること
                        entry['action'] = 'REVIEW_REQUIRED'
                        candidates.append(entry)
                        if verbose:
                            print(f'[REVIEW]    {pid}: {a_bool}→{expected} (conf={confidence:.2f}) ← 人間確認要')
    ...
    if apply and auto_fixed:
        ...
        with open(CSV_DIFF_PATH, 'w', encoding='utf-8', newline='') as f:
            writer = csv.DictWriter(
                f,
                fieldnames=['problemId', 'before_answer', 'after_answer',
                            'reason', 'confidence', 'applied_at'],
            )
            writer.writeheader()
            for e in auto_fixed:
                writer.writerow({
                    'problemId': e['problemId'],
                    'before_answer': e['storedAnswer'],
                    'after_answer': e['suggestedAnswer'],
                    'reason': e.get('direction', ''),
                    'confidence': e['confidence'],
                    'applied_at': applied_at,
                })
        print(f'[DIFF_CSV] Saved to {CSV_DIFF_PATH.name}')

    # ページ単位集計
    page_agg: dict[str, dict] = defaultdict(
        lambda: defaultdict(int)
    )
    def _page(pid: str) -> str:
        m = re.search(r'p(\d+)', pid)
        return f'p{m.group(1)}' if m else 'p???'

    for e in auto_fixed:
        page_agg[_page(e['problemId'])]['auto_fixed'] += 1
    for e in candidates:
        page_agg[_page(e['problemId'])]['candidates'] += 1
    for e in quality_flags:
        p = _page(e['problemId'])
    ...
    report = {
        ...
        'pageAggregation': {p: dict(c) for p, c in sorted(page_agg.items())},
        'summary': {
...
def print_summary(report: dict):
    ...
    print_page_summary(report)


def print_page_summary(report: dict):
    """ページ単位で異常件数を集計して表示（2件以上のページのみ）"""
    agg = report.get('pageAggregation', {})
    if not agg:
        return

    # ページごとの総件数を計算
    page_totals = {
        page: sum(counts.values())
        for page, counts in agg.items()
    }
    hot = {p: agg[p] for p, t in page_totals.items() if t >= 2}
    if not hot:
        return

    print()
    print('【ページ集計（2件以上）】')
    for page in sorted(hot.keys()):
        counts = hot[page]
        parts = []
        if counts.get('auto_fixed'):
            parts.append(f'自動修正:{counts["auto_fixed"]}件')
        if counts.get('candidates'):
            parts.append(f'要確認:{counts["candidates"]}件')
        # quality issues: anything else
        q_items = {k: v for k, v in counts.items()
                   if k not in ('auto_fixed', 'candidates')}
        for issue, cnt in sorted(q_items.items()):
            label = {
                'QUESTION_TRUNCATED': '途中切断',
                'EXPLANATION_TRUNCATED': '<省略>混入',
                'OCR_CORRUPTION_QUESTION': 'OCR崩壊',
                'OCR_CORRUPTION_EXPLANATION': 'OCR崩壊(解説)',
                'DESCRIPTIVE_QUESTION': '記述式',
                'EMPTY_QUESTION': '空問題',
                'TOO_SHORT_QUESTION': '極短問題',
                'LOW_SIMILARITY': '解説不一致',
            }.get(issue, issue)
            parts.append(f'{label}:{cnt}件')
...
if __name__ == '__main__':
    ...
    # 再監査: apply 後に再度チェックして残件数を表示
    if args.apply:
        print()
        print('─' * 55)
        print('【再監査 (apply後)】')
        report2 = run_optimizer(apply=False, verbose=False, _save_report=False)
        s1, s2 = report['summary'], report2['summary']
        print(f'  修正済み:   {s1["auto_fixed"]} 件 (今回適用)')
        print(f'  残・自動候補: {s2["auto_fixed"]} 件')
        print(f'  残・要確認:  {s2["review_candidates"]} 件')
        print(f'  残・品質フラグ: {s2["quality_flags"]} 件')
