...
**原因1: `importParsedBatch` が全削除→再挿入する**
- `refreshProblemDataIfNeeded` → `importParsedBatch` の流れで、対象ページの `problems` と `problemAttrs` を全削除してから再INSERT
- `reviewed_import.json` の `subjectCandidate` / `chapterCandidate` が `null` や `""` のブランチがあると分類が消える
- `DATA_VERSION` が上がるたびに発生リスクあり

**原因2: PATCHの除外フラグが再作成後に復元されない**
...
**原因3: `subjectId === ''` の問題が科目フィルタで死角になる**
- 分類が空の問題は「全科目表示」でしか見えなくなる
...
変更対象は OCR バッチ処理スクリプトのみ
- `scripts/ocr_batch.sh`
- または対応する Python スクリプト
...
- [ ] PATCH / cleanup の再実行条件を `DATA_VERSION` と整合させる
