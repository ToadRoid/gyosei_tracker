...
最終更新: 2026-04-20 (v92：A 群 #4 `p116-q01 seq2 E` を原本復元で close。頭欠落「再調査の請求においては、」+ OCR 誤字「処分庁が→は」+ 中盤 garble「申立てによる場合で…→申立てによりまたは職権で、…執行停止…をとることができる。」復元、行審法25条2項準用 + 61条、polarity 非影響。A 群 4 → 3、queue 10 → 9、次着手は A5 `p136-q01 seq1 E`)

**本ファイル単体で引き継ぎが成立することを目標にする**。repo 外 memory は補助扱い。

## 現在地（confirmed, 2026-04-20 v92 反映後）

* **main HEAD**: `a2b26111bdb818425109127b9f8717277407286c`（直前は handoff mainHEAD sync PR #48。本 PR #?? v92 merge 後に更新）
* **総ページ**: 239 / **総肢**: 1312（不変）
* **DATA_VERSION**: `2026-04-20-audit-v92-p116-q01-seq2-e-restore`
* **L1 本線**: ✅ 実質完了維持
* **直近 merge 履歴**: v88 (PR #43, `bd45bd7`) → v89 (PR #44, `2305dad`, A1 close) → v90 (PR #45, `e2321a9`, A2 close) → handoff sync (PR #46, `8b8c3b0`) → v91 (PR #47, `c9fe732`, A3 close) → handoff mainHEAD sync (PR #48, `a2b2611`) → v92 (PR #?? 予定, A4 close)
...
* **PR #?? / p116-q01 seq2 E restore**（A 群 #4、E text のみ、polarity 非影響、書籍 page 335 右列 row 8 ×）
...
* ~~A4 p116-q01 seq2 E（頭欠落「再調査の請求においては、」+ OCR 誤字「処分庁が→は」+ 中盤 garble「申立てによる場合で…→申立てによりまたは職権で、…執行停止…」復元、行審法25条2項 + 61条整合）~~ → v92 で close（PR #?? 予定）
...
| ~~A4~~ | ~~p116-q01 seq2 E~~ | ~~末尾 `…審査請求人の申立てによる場合で…` で中断、行審法25条2項準用の帰結欠落（+ 頭欠落 + 「処分庁が/は」OCR 誤字）~~ | ~~close（v92, PR #?? 予定）~~ |
| A5 | p136-q01 seq1 E | `提起すること...（行政事件訴訟法8条1項本文：自由選択主義）。` — 「ができる」等の語欠落 | 未 close |
| A6 | p136-q01 seq2 E | 末尾 `…自由選択主義...` で中断、結論文欠落 | 未 close |
| A7 | p136-q01 seq3 E | 末尾 `（行政事件訴訟法8条1項本文：自由選択主義）...` で中断 | 未 close |

**着手順**: 上表 ~~A1~~ → ~~A2~~ → ~~A3~~ → ~~A4~~ → A5 → A6 → A7（p175/p136 は同一ページ seq 近接配置。v87 で p136-q01 seq4 は close 済み、seq1/seq2/seq3 は別個の substantive 破損ゆえ 1 seq/1 PR が原則）。**次着手 = A5 `p136-q01 seq1 E`**（v92 で A4 close、A 群残 3 件、p175 / p162 / p116 に関する A 群項目はゼロ。A5/A6/A7 は p136 の seq1/seq2/seq3 で同一ページ、ただし 1 seq/1 PR 原則）。
...
**次アクション**: **A 群次着手 `A5 p136-q01 seq1 E`**（`提起すること...（行政事件訴訟法8条1項本文：自由選択主義）。` — 「ができる」等の語欠落、同 p136 内で seq1/seq2/seq3 が連続する別個 PR 対象）。A 群完了後に B 群の原本照合へ。C 群は修正不要ゆえ着手しない。別領域（`importParsedBatch` 分類継承バグ / `subjectId === ''` 禁止設計 / `needsSourceCheck` 自動検知 / OCR パイプラインモデル差し替え）への移行は **OCR pending queue が再び 0 に戻るまで保留**。
...
1. **累積 recheck queue 整理（再開中）** — v88 反映後に CLAUDE.md §5 auto-detection 再走で新規 hit 13 件を検出（A 群 7 件 = 実作業 / B 群 6 件 = 判断保留 / C 群 3 件 = 管理対象外）。**v89 A1 / v90 A2 / v91 A3 / v92 A4 close、次着手 = A5 `p136-q01 seq1 E`**（A 群残 3 / queue 全体 9、p175 / p162 / p116 に関する A 群項目はゼロ。残 3 件はすべて p136）。A 群完了後に B 群を原本照合で再判定。**OCR 系完了扱いへの移行は A 群・B 群を全て解消してから**。
