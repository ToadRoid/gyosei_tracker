...
最終更新: 2026-04-20 (v82 p227-q01 seq2 substantive restore 反映)

**本ファイル単体で引き継ぎが成立することを目標にする**。repo 外 memory は補助扱い。

## 現在地（confirmed, 2026-04-20 v82 反映後）

* **main HEAD**: `dad80289d4f6b6f48f459c75e94893826c7bafde`
* **総ページ**: 239 / **総肢**: 1312（不変）
* **DATA_VERSION**: `2026-04-20-audit-v82-p227-q01-seq2-substantive-restore`
* **L1 本線**: ✅ 実質完了維持
...
**列挙（5 件, 2026-04-20 v82 反映後）**:

| # | 対象 | 症状 | 状態 |
| --- | --- | --- | --- |
| 1 | p227-q01 seq3 E | `被補佐人` 周辺（同 E 内に `成年被後見人と成年被後見人` 重複、`13歳2項110号` 不明 garble 併発、単語差し替えで済まない） | OCR pending |
...
**次アクション**: queue の先頭（`p227-q01 seq3 E`）を原本読取（0227.png page 557 右列 row 1 / 意思表示の効力発生時期 Q1）で確定 → small PR。substantive 破損（`被補佐人` / `成年被後見人と成年被後見人` 重複 / `13歳2項110号` garble）のため、v82 と同様 E text + 条文照合の束ね修正になる可能性高い。
...
1. **累積 recheck queue 整理（最優先）** — OCR pending queue を 1 ページずつ page-by-page PR で消化（上記 §2 の 5 件、先頭は `p227-q01 seq3 E`）
