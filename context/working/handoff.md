...
## 現在地（confirmed, 2026-04-22 post-v99-merge + PR #67 merged + **v100 = B3 Q/E 層 1 restore + ans polarity flip 反映**）

* **latest data merge**: 本 PR = v100（PR # 未確定・merge 後に SHA 補完、B3 `p006-q01 seq3` Q/E 層 1 substantive restore + ans False→True polarity flip = 判例名補足 + 中盤 3 段 concrete text + 末尾「判示」正字化、層 2 fragment は `...` literal 保持で ERROR_UNREADABLE_SOURCE 運用）/ 直前の merged v## = v99 (`ed924f247ee6c780f5185b4921d06728fead65be`, PR #66, B2 Q/E 層 1 substantive restore + ans=False 維持)
* **latest main HEAD at handoff edit time**: `59cf4a1d2de1134a3e29095b8fdee045ac21ac1c`（PR #67 squash merge = B3 seq3 read-only sufficiency check handoff-only 更新 + v99 SHA backfill + エラー報告 queue 記録、本 PR 編集時の repo 先端。v100 squash SHA は merge 後に `latest data merge` に補完、次作業 PR で同期）
* **総ページ**: 239 / **総肢**: 1312（不変）
* **DATA_VERSION**: `2026-04-22-audit-v100-p006-q01-seq3-qe-restore-polarity-flip`（本 PR で v99 → v100 bump。`p006-q01 seq3` の ans を False → True flip + E を層 1 substantive restore、Q は無変更）
* **L1 本線**: ✅ 実質完了維持
* **直近 data merge 履歴**: v88 (PR #43, `bd45bd7`) → v89 (PR #44, `2305dad`, A1) → v90 (PR #45, `e2321a9`, A2) → v91 (PR #47, `c9fe732`, A3) → v92 (PR #49, `ece5d89`, A4) → v93 (PR #51, `075e4a2`, A5) → v94 (PR #54, `54ec51f`, A6) → v95 (PR #55, `95a50f9`, A7 = A 群完走) → v96 (PR #57, `a076e59`, B5) → v97 (PR #58, `7171dac`, **B6 = B 群 active 完走**) → v98 (PR #62, `671eb79`, **B2 polarity hotfix のみ = Q/E 保留**) → v99 (PR #66, `ed924f2`, **B2 Q/E 層 1 substantive restore、ans=False 維持**) → v100 (**本 PR**, B3 Q/E 層 1 substantive restore + ans False→True polarity flip 束ね、v82/v86 precedent = Q+E+ans bundled)
...
**次アクション**: **A 群完走** + **B 群 B6 まで close** + **別領域移行 #1 完了** + **B2 polarity hotfix（v98）完了** + **p006 preprocessed source 収載（PR #64, `4c19f08`）完了** + **B2 seq2 sufficiency check 完了（PR #65, `c0f8875`）** + **v99 = B2 seq2 Q/E 層 1 substantive restore 完了（PR #66, `ed924f2`）** + **B3 seq3 sufficiency check 完了（PR #67, `59cf4a1`）** + **v100 = B3 seq3 Q/E 層 1 substantive restore + ans False→True polarity flip 束ね完了（本 PR）**。次の優先順：
1. **B1 = p006-q01 seq1 Q + E**（優先順 `B2 → B3 → B1 → B4` に従い次着手、1 seq / 1 PR、同型 sufficiency → restore フロー）:
   - **手順**: (i) `images_preprocessed/0006.png` row 12 で read-only sufficiency check（PR #65 / PR #67 と同型、handoff-only）→ (ii) 判定が `restore 可` なら B1 verbatim restore PR（共産党除名事件、Q + E 両方に restore 要）
   - **read-only 心証（既存）**: 旧 handoff `cosmetic close` 判定は**誤り**（2026-04-22 user directive で再 open）、Q + E 両方の restore 要
2. **B4 = p006-q01 seq4 Q + E**（B1 後、1 seq / 1 PR、同型 sufficiency → restore フロー、在宅投票事件、E 中央 restore が主対象）。**B4 完了で p006 閉じる**
3. **エラー報告 queue 差し込み判定（v100 merge 後に再判定）**: #1 KB2025-p194-q06（正誤の誤り）の緊急度次第で B1/B4 前に差し込むか再判定。#2 #3 は B 群完走後に合流可能
