...
最終更新: 2026-04-20 (v91：A 群 #3 `p162-q01 seq2 E` を原本復元で close。OCR 1 char drop 2 箇所「という→といえる」「実質的に→実質的には」+ 文末欠落 → 国賠法3条1項共同執行者論 + 最判昭50.11.28 引用復元、polarity 非影響。A 群 5 → 4、queue 11 → 10、次着手は A4 `p116-q01 seq2 E`)

**本ファイル単体で引き継ぎが成立することを目標にする**。repo 外 memory は補助扱い。

## 現在地（confirmed, 2026-04-20 v91 反映後）

* **main HEAD**: `8b8c3b09295cb4371e41bac9e24432ebb3440b10`（直前は handoff 同期 PR #46。本 PR #?? v91 merge 後に更新）
* **総ページ**: 239 / **総肢**: 1312（不変）
* **DATA_VERSION**: `2026-04-20-audit-v91-p162-q01-seq2-e-restore`
* **L1 本線**: ✅ 実質完了維持
* **直近 merge 履歴**: v88 (PR #43, `bd45bd7`) → v89 (PR #44, `2305dad`, A1 close) → v90 (PR #45, `e2321a9`, A2 close) → handoff sync (PR #46, `8b8c3b0`) → v91 (PR #?? 予定, A3 close)
...
* **PR #?? / p162-q01 seq2 E restore**（A 群 #3、E text のみ、polarity 非影響、書籍 page 427 右列 row 2 ×）
...
* ~~A3 p162-q01 seq2 E（OCR 1 char drop 2 箇所「という→といえる」「実質的に→実質的には」+ 文末欠落 → 国賠法3条1項共同執行者論 + 最判昭50.11.28 引用復元）~~ → v91 で close（PR #?? 予定）
...
| ~~A3~~ | ~~p162-q01 seq2 E~~ | ~~末尾 `事業を共同して執行している…` で中断、国賠法3条1項共同執行者論の結語欠落（+ 前半に OCR 1 char drop 2 箇所「という/といえる」「実質的に/実質的には」）~~ | ~~close（v91, PR #?? 予定）~~ |
| A4 | p116-q01 seq2 E | 末尾 `…審査請求人の申立てによる場合で…` で中断、行審法25条2項準用の帰結欠落 | 未 close |
...
**着手順**: 上表 ~~A1~~ → ~~A2~~ → ~~A3~~ → A4 → A5 → A6 → A7（p175/p136 は同一ページ seq 近接配置。v87 で p136-q01 seq4 は close 済み、seq1/seq2/seq3 は別個の substantive 破損ゆえ 1 seq/1 PR が原則）。**次着手 = A4 `p116-q01 seq2 E`**（v91 で A3 close、A 群残 4 件、p175 / p162 に関する A 群項目はゼロ）。
...
**次アクション**: **A 群次着手 `A4 p116-q01 seq2 E`**（末尾 `…審査請求人の申立てによる場合で…` で中断、行審法25条2項準用の帰結欠落）。A 群完了後に B 群の原本照合へ。C 群は修正不要ゆえ着手しない。別領域（`importParsedBatch` 分類継承バグ / `subjectId === ''` 禁止設計 / `needsSourceCheck` 自動検知 / OCR パイプラインモデル差し替え）への移行は **OCR pending queue が再び 0 に戻るまで保留**。
...
1. **累積 recheck queue 整理（再開中）** — v88 反映後に CLAUDE.md §5 auto-detection 再走で新規 hit 13 件を検出（A 群 7 件 = 実作業 / B 群 6 件 = 判断保留 / C 群 3 件 = 管理対象外）。**v89 A1 / v90 A2 / v91 A3 close、次着手 = A4 `p116-q01 seq2 E`**（A 群残 4 / queue 全体 10、p175 / p162 に関する A 群項目はゼロ）。A 群完了後に B 群を原本照合で再判定。**OCR 系完了扱いへの移行は A 群・B 群を全て解消してから**。
