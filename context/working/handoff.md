...
最終更新: 2026-04-20 (v81 p219-q01 seq3 Q 復元反映)

**本ファイル単体で引き継ぎが成立することを目標にする**。repo 外 memory は補助扱い。

## 現在地（confirmed, 2026-04-20 v81 反映後）

* **main HEAD**: `101679029f62daafd08e79a063c7621db30cbfad`
* **総ページ**: 239 / **総肢**: 1312（不変）
* **DATA_VERSION**: `2026-04-20-audit-v81-p219-q01-seq3-q-restore`
* **L1 本線**: ✅ 実質完了維持
...
**列挙（6 件, 2026-04-20 v81 反映後）**:

| # | 対象 | 症状 | 状態 |
| --- | --- | --- | --- |
| 1 | p227-q01 seq2 Q | `知ることができる適当先がなかった` （単純 typo ではなく文構造ごと破損疑い、第三者強迫主語も揺れ） | OCR pending |
| 2 | p227-q01 seq3 E | `被補佐人` 周辺（同 E 内に `成年被後見人と成年被後見人` 重複、`13歳2項110号` 不明 garble 併発、単語差し替えで済まない） | OCR pending |
| 3 | p062-q01 seq7 Q | Q 文末欠落疑い（`...` / `…` を含む） | OCR pending |
| 4 | p078-q01 seq4 Q | Q 文末欠落疑い | OCR pending |
| 5 | p119-q01 seq1 Q | Q 文末欠落疑い | OCR pending |
| 6 | p136-q01 seq4 Q | Q 文末欠落疑い | OCR pending |

**件数増減のルール**: 新たな scan hit や user 指摘で件数が増減する場合、理由を 1 行ずつ本表末に追記する。

- 2026-04-20: v81 反映で `p219-q01 seq3 Q` を close（-1 → 6 件）

**次アクション**: queue の先頭（`p227-q01 seq2 Q`）を原本読取（0227.png）で確定 → Q text のみ修正の small PR。
...
1. **累積 recheck queue 整理（最優先）** — OCR pending queue を 1 ページずつ page-by-page PR で消化（上記 §2 の 6 件、先頭は `p227-q01 seq2 Q`）
