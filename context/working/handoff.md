...
最終更新: 2026-04-20 (v90：A 群 #2 `p175-q01 seq3 E` を原本復元で close。条番号 garble 102条1項→2項 + 接続詞 また→なお + 構造/語句 garble 議会（…妥当）→議会は…原則であるが + 文末欠落 → 地方自治法102条全項 + 102条の2第1項 整合、polarity 非影響。A 群 6 → 5、queue 12 → 11、次着手は A3 `p162-q01 seq2 E`)
...
* **PR #?? / p175-q01 seq3 E restore**（A 群 #2、E text のみ、polarity 非影響、書籍 page 453 右列 row 3 ×）
...
* ~~A2 p175-q01 seq3 E（条番号 garble 102条1項→2項 + 接続詞「また→なお」+ 構造 garble「議会（…妥当であるが）」→「議会は…原則であるが（102条1項）」+ 文末欠落 → 通年会期制 102条の2第1項 まで復元、地方自治法102条全項整合）~~ → v90 で close（PR #?? 予定）
...
| ~~A2~~ | ~~p175-q01 seq3 E~~ | ~~末尾 `（定例会および臨時会とするのが妥当であるが）...` で中断（+ 前半に 102条1項/2項 条番号 garble、「また/なお」接続詞 garble、括弧構造 + 妥当/原則 garble）~~ | ~~close（v90, PR #?? 予定）~~ |
| A3 | p162-q01 seq2 E | 末尾 `事業を共同して執行している…` で中断、国賠法3条1項共同執行者論の結語欠落 | 未 close |
...
**着手順**: 上表 ~~A1~~ → ~~A2~~ → A3 → A4 → A5 → A6 → A7（p175/p136 は同一ページ seq 近接配置。v87 で p136-q01 seq4 は close 済み、seq1/seq2/seq3 は別個の substantive 破損ゆえ 1 seq/1 PR が原則）。**次着手 = A3 `p162-q01 seq2 E`**（v90 で A2 close、A 群残 5 件、p175 は seq1/seq3 両方 close 済みで p175 に関する A 群項目はゼロ）。
...
**次アクション**: **A 群次着手 `A3 p162-q01 seq2 E`**（末尾 `事業を共同して執行している…` で中断、国賠法3条1項共同執行者論の結語欠落）。A 群完了後に B 群の原本照合へ。C 群は修正不要ゆえ着手しない。別領域（`importParsedBatch` 分類継承バグ / `subjectId === ''` 禁止設計 / `needsSourceCheck` 自動検知 / OCR パイプラインモデル差し替え）への移行は **OCR pending queue が再び 0 に戻るまで保留**。
...
1. **累積 recheck queue 整理（再開中）** — v88 反映後に CLAUDE.md §5 auto-detection 再走で新規 hit 13 件を検出（A 群 7 件 = 実作業 / B 群 6 件 = 判断保留 / C 群 3 件 = 管理対象外）。**v89 で A1 close / v90 で A2 close、次着手 = A3 `p162-q01 seq2 E`**（A 群残 5 / queue 全体 11、p175 に関する A 群項目はゼロ）。A 群完了後に B 群を原本照合で再判定。**OCR 系完了扱いへの移行は A 群・B 群を全て解消してから**。
