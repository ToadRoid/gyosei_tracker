...
最終更新: 2026-04-22 (**v99 = B2 p006-q01 seq2 Q/E verbatim restore**：PR #65 sufficiency check 結論 `restore 可（B5/B6 precedent = 二層運用）` を受けて層 1 = substantive restore を実施。E 冒頭「最判昭35.6.8」→「**最大判**昭35.6.8」1 char 補正 + E 中盤 `...` → 3 段構造 concrete text restore（直接国家統治 / 法律上の争訟 / 裁判所の審査権の外）+ E 末尾「〜という**本肢は誤っている**。」評価句 restore。Q は構造レベルで verbatim 一致と判読、Layer 1 では未変更。ans=False は v98 hotfix から維持。**二層表現採用**：層 1 = substantive risk mitigated（本 PR）／ 層 2 = image-quality-limited fragment（助詞 / 句読点 / subject 語句 marginal）は restore E 本文中に `...` literal を保持し **ERROR_UNREADABLE_SOURCE** として維持（B5 `0090.png` 中盤 / B6 `0118.png` 84条繋ぎ末尾と同一運用）。PR #64 = `4c19f0843f874d93a07811c6d010d7694610411e` と PR #65 = `c0f887533e3e8533bd90e9e1decc60f547e23328` の squash SHA を backfill 同梱。DATA_VERSION v98 → v99 bump。他 seq (1/3/4) / 他ページ hash 変化なし、mirror byte-identical。) / 旧: 2026-04-22 (**v98 polarity hotfix**：`p006-q01 seq2` の `answerBoolean` を True → False に単独 flip。現 DB が Q「留保あり、司法審査は及ばない」/ E「留保は付されていない」で自己矛盾していたため、polarity のみ先行修正。**Q / E は未修正維持**。理由：p006 原本画像（`images/0006.png` / `images_preprocessed/0006.png`）が repo 内に未存在で、Q/E の verbatim restore は「推測禁止 / repo 実状態優先」ルールに抵触するため保留。**二層表現採用**：層 1 = polarity hotfix（ans flip 完了）／ 層 2 = Q/E verbatim restore 保留（原本画像到着待ち）。他 seq (1/3/4) 完全不変、他 seq の hash 変化なし確認済。DATA_VERSION v97 → v98 bump。)
...
## 現在地（confirmed, 2026-04-22 post-v98-merge + PR #63 + PR #64 + PR #65 merged + **v99 = B2 Q/E 層 1 restore 反映**）

* **latest data merge**: 本 PR = v99（PR # 未確定・merge 後に SHA 補完、B2 `p006-q01 seq2` Q/E 層 1 substantive restore = 最判→最大判 + E 中盤 3 段 concrete text + E 末尾「本肢は誤っている」評価句、ans=False 維持、層 2 fragment は `...` literal 保持で ERROR_UNREADABLE_SOURCE 運用）/ 直前の merged v## = v98 (`671eb79f622bb797ec78b2c97788edba9ebe7b54`, PR #62, B2 ans polarity hotfix 先行 = Q/E 未修正維持)
* **latest main HEAD at handoff edit time**: `c0f887533e3e8533bd90e9e1decc60f547e23328`（PR #65 squash merge = B2 seq2 read-only sufficiency check handoff-only 更新、本 PR 編集時の repo 先端。v99 squash SHA は merge 後に `latest data merge` に補完、次作業 PR で同期）
* **総ページ**: 239 / **総肢**: 1312（不変）
* **DATA_VERSION**: `2026-04-22-audit-v99-p006-q01-seq2-qe-restore`（本 PR で v98 → v99 bump。`p006-q01 seq2` の E を層 1 substantive restore、Q / ans は v98 維持）
* **L1 本線**: ✅ 実質完了維持
* **直近 data merge 履歴**: v88 (PR #43, `bd45bd7`) → v89 (PR #44, `2305dad`, A1) → v90 (PR #45, `e2321a9`, A2) → v91 (PR #47, `c9fe732`, A3) → v92 (PR #49, `ece5d89`, A4) → v93 (PR #51, `075e4a2`, A5) → v94 (PR #54, `54ec51f`, A6) → v95 (PR #55, `95a50f9`, A7 = A 群完走) → v96 (PR #57, `a076e59`, B5) → v97 (PR #58, `7171dac`, **B6 = B 群 active 完走**) → v98 (PR #62, `671eb79`, **B2 polarity hotfix のみ = Q/E 保留**) → v99 (**本 PR**, B2 Q/E 層 1 substantive restore = 最判→最大判 + `...` → 3 段 concrete + 末尾評価句、層 2 marginal は ERROR_UNREADABLE_SOURCE 運用、ans=False 維持)
* **v99 PR 内 SHA backfill**: PR #64 squash = `4c19f0843f874d93a07811c6d010d7694610411e`（p006 preprocessed source 収載）、PR #65 squash = `c0f887533e3e8533bd90e9e1decc60f547e23328`（B2 read-only sufficiency check）を本 PR に同梱 backfill（new policy PR #53 準拠、sync-only PR なし）
...
- 2026-04-22: **v99 = B2 p006-q01 seq2 Q/E verbatim restore**（本 PR、data 変更あり、DATA_VERSION v98 → v99 bump、ans=False 維持、1 seq / 1 PR）:
...
**次アクション**: **A 群完走** + **B 群 B6 まで close** + **別領域移行 #1 完了** + **B2 polarity hotfix（v98）完了** + **post-v98-merge handoff-only 更新（PR #63, `0c6ea3b`）完了** + **p006 preprocessed source 収載（PR #64, `4c19f08`）完了** + **B2 seq2 read-only sufficiency check 完了（PR #65, `c0f8875`, 結論 = `restore 可（B5/B6 precedent = 二層運用）`）** + **v99 = B2 seq2 Q/E 層 1 substantive restore 完了（本 PR）**。次の優先順：
1. **B3 = p006-q01 seq3 E + ans（polarity flip 候補）**（優先順 `B2 → B3 → B1 → B4` に従い次着手、1 seq / 1 PR）:
   - **手順**: (i) `images_preprocessed/0006.png` row 14 で read-only sufficiency check（PR #65 と同型、handoff-only）→ (ii) 判定が `restore 可` なら B3 verbatim restore PR（ans polarity 確認 + E 中盤 substantive restore）
   - **read-only 心証（PR #65 時点で未着手）**: Q「〜司法審査の対象にならない」= 統治行為論の肯定、判例整合なら ans=True 候補（現 DB=False）。ただし確定は sufficiency check 後
   - **OCR 方針 2026-04-22（三者分担）との整合**: Gemini を一次 OCR、Claude を実装・記録、GPT をレビュー担当とする運用で回す
2. **B1 = p006-q01 seq1 Q + E**（B3 後、1 seq / 1 PR、同型 sufficiency → restore フロー）
3. **B4 = p006-q01 seq4 Q + E**（B1 後、1 seq / 1 PR、同型 sufficiency → restore フロー）
4. **（分岐, 未採用）strict verbatim policy を採用する場合**: 層 2 fragment 保留も許さないならば B3/B1/B4 着手前に「真の high-res recrop 取得」に切り替え（low-res raw `~/Desktop/kindle_shots/0006.png` 再撮影 / 書籍原本からの再 capture）。本 v99 で確立した B5/B6/B2 precedent は **二層運用（層 1 restore + 層 2 `...` literal 保持）** で統一済
