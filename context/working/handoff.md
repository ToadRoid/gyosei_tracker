...
最終更新: 2026-04-20 (v88：p227-q01 seq3 E 条文引用 digit 高解像度 crop 復元、polarity 非影響、**OCR pending queue 完全空（実作業・外部確認待ちとも 0 件）**)
...
**列挙（0 件, 2026-04-20 v88 反映後）**:

| # | 対象 | 症状 | 状態 |
| --- | --- | --- | --- |
| — | — | — | — |

**OCR pending queue 完全空（実作業・外部確認待ちとも 0 件）**。v88 で v83 保留の最終 1 件（`p227-q01 seq3 E 条文引用` digit garble）を close。新規 scan hit / user 指摘があれば本表に追記して再開する。
...
**次アクション**: OCR pending queue 完全空。新規 scan hit / user 指摘があれば §2 に追記して再開。並行して「次に触るべき領域」（§下記）から方針判断へ（候補：(a) 新規 scan hit 再探索 — CLAUDE.md §5 auto-detection rule、(b) 別領域移行 — `importParsedBatch` 分類継承バグ / `subjectId === ''` 禁止設計 / `needsSourceCheck` 自動検知 / OCR パイプラインモデル差し替え）。
...
1. **累積 recheck queue 整理（完全完了）** — OCR pending queue は v88 で完全空（実作業・外部確認待ちとも 0 件）。v83 保留の最終 1 件（`p227-q01 seq3 E` digit garble）を高解像度 crop で close。新規 scan hit / user 指摘があれば §2 に追記して再開。
