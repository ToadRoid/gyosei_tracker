...
## 現在地（confirmed, 2026-04-22 post-v99-merge + **B3 sufficiency check 判定済 = restore 可 + ans False→True flip 候補**）

* **latest data merge**: `ed924f247ee6c780f5185b4921d06728fead65be`（v99 squash merge, PR #66, B2 `p006-q01 seq2` Q/E 層 1 substantive restore = 最判→最大判 + E 中盤 3 段 concrete text + E 末尾「本肢は誤っている」評価句、ans=False 維持、層 2 fragment は `...` literal 保持で ERROR_UNREADABLE_SOURCE 運用）/ 直前の merged v## = v98 (`671eb79f622bb797ec78b2c97788edba9ebe7b54`, PR #62, B2 ans polarity hotfix 先行 = Q/E 未修正維持)
* **latest main HEAD at handoff edit time**: `ed924f247ee6c780f5185b4921d06728fead65be`（v99 squash merge 後の repo 先端、本 PR = B3 sufficiency check handoff-only 更新は main HEAD を進めない）
* **総ページ**: 239 / **総肢**: 1312（不変）
* **DATA_VERSION**: `2026-04-22-audit-v99-p006-q01-seq2-qe-restore`（v99 で v98 → v99 bump、本 PR = handoff-only ゆえ維持）
* **L1 本線**: ✅ 実質完了維持
* **直近 data merge 履歴**: v88 (PR #43, `bd45bd7`) → v89 (PR #44, `2305dad`, A1) → v90 (PR #45, `e2321a9`, A2) → v91 (PR #47, `c9fe732`, A3) → v92 (PR #49, `ece5d89`, A4) → v93 (PR #51, `075e4a2`, A5) → v94 (PR #54, `54ec51f`, A6) → v95 (PR #55, `95a50f9`, A7 = A 群完走) → v96 (PR #57, `a076e59`, B5) → v97 (PR #58, `7171dac`, **B6 = B 群 active 完走**) → v98 (PR #62, `671eb79`, **B2 polarity hotfix のみ = Q/E 保留**) → v99 (PR #66, `ed924f2`, **B2 Q/E 層 1 substantive restore = 最判→最大判 + `...` → 3 段 concrete + 末尾評価句、層 2 marginal は ERROR_UNREADABLE_SOURCE 運用、ans=False 維持**)
...
| B3 | p006-q01 seq3 E + ans | 統治行為論、`判例は...場合でも` 中間語脱落、**ans False→True flip 候補**（read-only 判定時点の心証。判例特定・verbatim restore 詳細は原本画像到着後に確定） | **sufficiency check 済 = `restore 可（B5/B6/B2 precedent = 二層運用、+ ans polarity flip 同 PR 内束ね候補）`**（2026-04-22, 本 PR, `images_preprocessed/0006.png` row 14 による read-only 判定。画像 row 14 右列 marker = `○` → Q「司法審査の対象にならない」= TRUE で苫米地判例と整合、現 DB `ans=false` は polarity 逆 → 次 PR で True に flip。E 骨格は「いわゆる統治行為の問題である。判例（最大判昭35.6.8：苫米地事件）は〜かかる国家行為は裁判所の審査の対象とならないと判示している。」と判読可、現 DB の冒頭「統治行為論の問題であり、」+ 中盤 `...` + 末尾「判断している」を restore 対象。character-level marginal fragment は次 PR で `...` literal 保持 = ERROR_UNREADABLE_SOURCE 扱い = B5/B6/B2 同運用。**次 PR = v100 = B3 seq3 verbatim restore + ans polarity flip 束ね**） |
...
- 2026-04-22: **B3 p006-q01 seq3 read-only sufficiency check + v99 merge SHA backfill + エラー報告 queue 記録**（本 PR, handoff-only 更新, data 変更なし / DATA_VERSION bump なし / v99 維持 / Q/E/ans 無変更）:
...
**次アクション**: **A 群完走** + **B 群 B6 まで close** + **別領域移行 #1 完了** + **B2 polarity hotfix（v98）完了** + **p006 preprocessed source 収載（PR #64, `4c19f08`）完了** + **B2 seq2 sufficiency check 完了（PR #65, `c0f8875`）** + **v99 = B2 seq2 Q/E 層 1 substantive restore 完了（PR #66, `ed924f2`）** + **B3 seq3 read-only sufficiency check 完了（本 PR, 結論 = `restore 可（B5/B6/B2 precedent = 二層運用、+ ans polarity flip 同 PR 内束ね候補）`）**。次の優先順：
1. **v100 = B3 seq3 Q/E verbatim restore + ans polarity flip 束ね**（ans False→True、1 seq / 1 PR、v82/v86 precedent の Q+E+ans bundled 束ねパターン）:
   - **層 1 restore 範囲**: (i) ans False→True polarity flip（row 14 marker `○` + 判例整合）、(ii) E 冒頭「統治行為論の問題であり、判例は」→「いわゆる統治行為の問題である。判例（最大判昭35.6.8：苫米地事件）は、」（判例名補足 + 語尾 + 句点）、(iii) E 中盤 `...` → 3 段 concrete text restore（直接国家統治 / 法律上の争訟 / 裁判所の審査の対象）、(iv) E 末尾「判断している」→「判示している」（動詞 1 char 正字化）
   - **層 2 ERROR_UNREADABLE_SOURCE 扱い**: 助詞レベル（「のごときは」句読点、「たとえ」有無）/ subject 語句 character-level marginal / 句読点細部
   - **v100 PR 内で backfill する SHA**: 本 PR（B3 sufficiency check）の squash merge SHA
   - **Q は変更なし**（画像 row 14 左と現 DB Q が verbatim 一致と judge）
2. **エラー報告 queue 差し込み判定（v100 完了後）**: #1 KB2025-p194-q06（正誤の誤り）の緊急度次第で B1/B4 前に差し込むか再判定。#2 #3 は B 群完走後に合流可能
3. **B1 = p006-q01 seq1 Q + E**（v100 + 差し込み判定後、1 seq / 1 PR、同型 sufficiency → restore フロー）
4. **B4 = p006-q01 seq4 Q + E**（B1 後、1 seq / 1 PR、同型 sufficiency → restore フロー）
5. **（分岐, 未採用）strict verbatim policy を採用する場合**: 層 2 fragment 保留も許さないならば v100 / B1 / B4 着手前に「真の high-res recrop 取得」に切り替え。本 v96-v99 で確立した B5/B6/B2 precedent は **二層運用（層 1 restore + 層 2 `...` literal 保持）** で統一済
