...
最終更新: 2026-04-20 (v89：A 群 #1 `p175-q01 seq1 E` を原本復元で close。OCR 誤字 2 箇所 + 条番号 garble + 文末欠落 → 地方自治法101条1項-6項の正文、polarity 非影響。A 群 7 → 6、次着手は A2 `p175-q01 seq3 E`。v88 反映後の §5 再走で queue は 0 → 13 に復帰済み)
...
* **PR #?? / p175-q01 seq1 E restore**（A 群 §5 再走検出分の先頭、E text のみ、polarity 非影響、書籍 page 453 右列 row 1 ×）
...
* ~~A1 p175-q01 seq1 E（§5 再走検出分の先頭、OCR 誤字 2 箇所「議案→議長」「ときは→ときも」+ 条番号 garble 104条4項→101条4項 + 文末欠落 `。も...` → 101条5項/6項まで復元、地方自治法101条全項整合）~~ → v89 で close（PR #?? 予定）
...
| # | 対象 | 症状 | 状態 |
| --- | --- | --- | --- |
| ~~A1~~ | ~~p175-q01 seq1 E~~ | ~~末尾 `（104条4項）。も...` で dangling（「も...」だけ残存）~~ | ~~close（v89, PR #?? 予定）~~ |
| A2 | p175-q01 seq3 E | 末尾 `（定例会および臨時会とするのが妥当であるが）...` で中断 | 未 close |
...
**着手順**: 上表 ~~A1~~ → A2 → A3 → A4 → A5 → A6 → A7（p175/p136 は同一ページ seq 近接配置。v87 で p136-q01 seq4 は close 済み、seq1/seq2/seq3 は別個の substantive 破損ゆえ 1 seq/1 PR が原則）。**次着手 = A2 `p175-q01 seq3 E`**（v89 で A1 close、A 群残 6 件）。
...
**次アクション**: **A 群次着手 `A2 p175-q01 seq3 E`**（末尾 `（定例会および臨時会とするのが妥当であるが）...` で中断、同 p175 内ゆえ原本同ページで継続照合可能）。A 群完了後に B 群の原本照合へ。C 群は修正不要ゆえ着手しない。別領域（`importParsedBatch` 分類継承バグ / `subjectId === ''` 禁止設計 / `needsSourceCheck` 自動検知 / OCR パイプラインモデル差し替え）への移行は **OCR pending queue が再び 0 に戻るまで保留**。
...
1. **累積 recheck queue 整理（再開中）** — v88 反映後に CLAUDE.md §5 auto-detection 再走で新規 hit 13 件を検出（A 群 7 件 = 実作業 / B 群 6 件 = 判断保留 / C 群 3 件 = 管理対象外）。**A 群先頭 `p175-q01 seq1 E` から着手**（着手順は §2 の表参照）。A 群完了後に B 群を原本照合で再判定。**OCR 系完了扱いへの移行は A 群・B 群を全て解消してから**。
