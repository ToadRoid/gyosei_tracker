...
最終更新: 2026-04-20 (v93：A 群 #5 `p136-q01 seq1 E` を原本復元で close。中盤 substantive garble「定めがなければ、→法令の規定により審査請求をすることができる場合においても、」+ duplicate phrase 除去 + 文末欠落「提起すること...→提起することができる」、行訴法8条1項本文: 自由選択主義、polarity 非影響。A 群 3 → 2、queue 9 → 8、次着手は A6 `p136-q01 seq2 E`)
...
* **L1 本線**: ✅ 実質完了維持
* **直近 merge 履歴**: v88 (PR #43, `bd45bd7`) → v89 (PR #44, `2305dad`, A1 close) → v90 (PR #45, `e2321a9`, A2 close) → handoff sync (PR #46, `8b8c3b0`) → v91 (PR #47, `c9fe732`, A3 close) → handoff mainHEAD sync (PR #48, `a2b2611`) → v92 (PR #49, `ece5d89`, A4 close) → handoff mainHEAD sync (PR #50, `8286ebb`) → v93 (PR #?? 予定, A5 close)
...
* **PR #?? / p136-q01 seq1 E restore**（A 群 #5、E text のみ、polarity 非影響、書籍 page 375 右列 row 1 ○）
...
* ~~A5 p136-q01 seq1 E（中盤 substantive garble「定めがなければ、→法令の規定により審査請求をすることができる場合においても、」+ duplicate phrase 除去 + 文末欠落「提起すること...→提起することができる」、行訴法8条1項本文: 自由選択主義）~~ → v93 で close（PR #?? 予定）
...
| ~~A5~~ | ~~p136-q01 seq1 E~~ | ~~`提起すること...（行政事件訴訟法8条1項本文：自由選択主義）。` — 「ができる」等の語欠落（+ 中盤 substantive garble「定めがなければ」、duplicate phrase）~~ | ~~close（v93, PR #?? 予定）~~ |
| A6 | p136-q01 seq2 E | 末尾 `…自由選択主義...` で中断、結論文欠落 | 未 close |
| A7 | p136-q01 seq3 E | 末尾 `（行政事件訴訟法8条1項本文：自由選択主義）...` で中断 | 未 close |

**着手順**: 上表 ~~A1~~ → ~~A2~~ → ~~A3~~ → ~~A4~~ → ~~A5~~ → A6 → A7（p175/p136 は同一ページ seq 近接配置。v87 で p136-q01 seq4 は close 済み、seq1/seq2/seq3 は別個の substantive 破損ゆえ 1 seq/1 PR が原則）。**次着手 = A6 `p136-q01 seq2 E`**（v93 で A5 close、A 群残 2 件、いずれも p136。A7 = seq3）。
...
**次アクション**: **A 群次着手 `A6 p136-q01 seq2 E`**（末尾 `…自由選択主義...` で中断、結論文全滅）。同 p136 内で連続 PR 対象（1 seq/1 PR 原則堅持）。A 群完了後に B 群の原本照合へ。C 群は修正不要ゆえ着手しない。別領域（`importParsedBatch` 分類継承バグ / `subjectId === ''` 禁止設計 / `needsSourceCheck` 自動検知 / OCR パイプラインモデル差し替え）への移行は **OCR pending queue が再び 0 に戻るまで保留**。
...
1. **累積 recheck queue 整理（再開中）** — v88 反映後に CLAUDE.md §5 auto-detection 再走で新規 hit 13 件を検出（A 群 7 件 = 実作業 / B 群 6 件 = 判断保留 / C 群 3 件 = 管理対象外）。**v89 A1 / v90 A2 / v91 A3 / v92 A4 / v93 A5 close、次着手 = A6 `p136-q01 seq2 E`**（A 群残 2 / queue 全体 8、p175 / p162 / p116 に関する A 群項目はゼロ。残 2 件はすべて p136 seq2/seq3）。A 群完了後に B 群を原本照合で再判定。**OCR 系完了扱いへの移行は A 群・B 群を全て解消してから**。
